...
#'       exit=0,
#'       count=0)
#' lpo2=iterate_4x7dual_process(lpo)
#' Questions: why does it take 20 iterations of the while loop in dual assign to complete when everyone is assigned by count = 5
#' what does y index represent
...
iterate_dual_process=function(lpo){
  ...
  # y [  0  | diag(otperform)   ]
  # indices for the demand constraint columns
  eta.index=c((lpo$n[3]+1):(lpo$n[3]+lpo$n[1])) 
  # indicies for the columns in the utilization matrix 
  theta.index=c(1:lpo$n[3]) 
  # indicies for the rows in the utilization matrix
  x.index=c(1:(lpo$n[3]*lpo$n[2])) 
  # why is the y.index go from 16 to 10 
