...
dual_assignment=function(ss){
  ...
  lpo=list(
    ...
    ass=c(rep(0,n[3])),
    bl=c(rep(0,n[1])),
    istar=NULL, #istar is what updates to let the program know which person has been assigned in numUsers
    jstar=NULL, # jstar points to the task someone had gotten 
    cost=0,
    exit=0,
    count=0)
